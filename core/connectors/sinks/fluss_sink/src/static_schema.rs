...
impl From<Error> for IggyError {
    fn from(error: Error) -> Self {
        ...
        match error {
            ...
            Error::InvalidPayloadUtf8 { .. } | Error::PayloadFormatNotSupported { .. } => {
                Self::Serialization(message)
            }
            Error::TimestampConversion { .. } | Error::PayloadBytesConversion { .. } => {
...
impl SingleTableBatchBuilder {
    ...
    pub(crate) fn finish(&mut self, context: &RowContext) -> RecordBatch {
...
mod tests {
    ...
    fn given_all_extra_columns_when_converting_messages_should_preserve_array_contents() {
        let layout = SingleTableLayout::from_config(&test_config(PayloadFormat::Text));
    ...
    fn given_invalid_utf8_before_valid_message_when_encoding_arrow_rows_should_skip_invalid_row() {
        ...
        let rows = builder.finish(&test_context());
        assert_eq!(errors, 1);
    ...
    fn given_json_payload_when_building_arrow_batch_should_serialize_payload_as_string() {
        ...
        let rows = build_arrow_rows(&layout, vec![test_message(payload)]);
    ...
    fn given_max_unsigned_values_when_building_arrow_batch_should_preserve_values() {
        let layout = SingleTableLayout::from_config(&test_config(PayloadFormat::Bytea));
        let mut message = test_message(Payload::Raw(vec![1]));
        message.id = u128::MAX;
        message.offset = u64::MAX;
        message.checksum = u64::MAX;
        let rows = build_arrow_rows(&layout, vec![message]);
