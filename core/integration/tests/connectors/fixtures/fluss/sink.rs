...
use crate::connectors::{TestMessage, fixtures::fluss::fixture_error};

use super::cluster::FlussCluster;
use async_trait::async_trait;
use fluss::row::{ColumnarRow, GenericRow};
use fluss::{
...
const ENV_SINK_PAYLOAD_FORMAT: &str = "IGGY_CONNECTORS_SINK_FLUSS_PLUGIN_CONFIG_PAYLOAD_FORMAT";
...
fn create_test_table_schema() -> Result<Schema, TestBinaryError> {
    let schema = Schema::builder()
        ...
        .build()
        .map_err(|e| fixture_error(format!("Failed to build schema: {}", e)))?;

    Ok(schema)
}

fn test_message_to_fluss_row(message: &TestMessage) -> GenericRow<'_> {
    let mut row = GenericRow::new(6);
    row.set_field(0, message.id as i64);
    row.set_field(1, message.name.clone());
    row.set_field(2, message.count as i32);
    row.set_field(3, message.amount);
    row.set_field(4, message.active);
    row.set_field(5, message.timestamp);
    row
}
...
impl FlussSinkFixture {
    ...
    pub async fn write_to_test_table(
        &self,
        ...
        let appender = table
            .new_append()
            .map_err(|e| fixture_error(format!("Failed to create appender: {}", e)))?
            .create_writer()
            .map_err(|e| fixture_error(format!("Failed to create writer: {}", e)))?;

        for message in messages {
            let row = test_message_to_fluss_row(message);

            appender
                .append(&row)
                .map_err(|e| fixture_error(format!("Failed to append row: {}", e)))?;
        }

        appender
            .flush()
            .await
            .map_err(|e| fixture_error(format!("Failed to flush appender: {}", e)))?;
    ...
    pub async fn create_test_table(&self) -> Result<TablePath, TestBinaryError> {
        let connection = self
            .get_fluss_connection()
            .await
            .map_err(|e| fixture_error(format!("Error getting Fluss connection {}", e)))?;
        ...
        let descriptor = TableDescriptor::builder()
            .schema(schema)
            .build()
            .map_err(|e| fixture_error(format!("Error building table descriptor: {}", e)))?;
        admin
            .create_table(&table_path, &descriptor, true)
            .await
            .map_err(|e| fixture_error(format!("Error creating table: {}", e)))?;
...
impl TestFixture for FlussSinkFixture {
    ...
    fn connectors_runtime_envs(&self) -> HashMap<String, String> {
        HashMap::from([
            ...
            (ENV_SINK_PAYLOAD_FORMAT.to_string(), "bytea".to_string()),
            (
